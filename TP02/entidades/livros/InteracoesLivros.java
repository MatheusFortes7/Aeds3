...
public class InteracoesLivros {
  ...
  private ArquivoLivro arqLivros;
  ...
  public InteracoesLivros() {
    try {
      ...
      if (!d.exists())
        d.mkdir();
      arqLivros = new ArquivoLivro("dados/livros");
    } catch (Exception e) {
  ...
  public void incluirLivro() {
    ...
    try {
      ...
      do {
        novoLivro = leLivro();
        if (novoLivro.getTitulo().length() == 0 || novoLivro.getAutor().length() == 0 || novoLivro.getPreco() < 0
  ...
  public void buscarLivroTitulo(){
    ...
    try {
      Livro livro = arqLivros.read(titulo);
      if(livro != null){
        mostraLivro(livro);
      } else {
  ...
  public void alterarLivro() {
    ...
    try {
      ...
      if (l2.getTitulo().length() > 0)
        l.setTitulo(l2.getTitulo());
      if (l2.getAutor().length() > 0)
        l.setAutor(l2.getAutor());
      if (l2.getPreco() > 0)
        l.setPreco(l2.getPreco());
      if (l2.getLancamento().getTime() > 0)
        l.setLancamento(l2.getLancamento().getTime());
